...
FOLDERS = %w(colors ftdetect ftplugin indent syntax doc plugin autoload snippets macros after ruby compiler)
SCRIPTS = %w(jasmine.vim personal Tagbar vim-rvm Headlights vim-coffee-script tabular nerdtree vim-cucumber vim-rails vim-fugitive vim-haml vim-scratch ack.vim snipmate.vim vim-spec tcomment_vim vim-bufonly vim-endwise vim-surround vim-yankring supertab vim-unimpaired Slimv vim-rake vim-javascript vim-autoclose vim-ruby-refactoring matchit cscope gundo.vim vim-conque vim-repeat vim-space vim-colors-solarized vim-indent-guides vim-less vim-fireplace vim-classpath vim-clojure-static) + SCRIPTS_WITH_RAKE
DOTVIM = "#{ENV['HOME']}/.vim"
